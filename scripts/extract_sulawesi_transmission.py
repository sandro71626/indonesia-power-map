"""
Build transmission_master_sulawesi.csv + GeoJSON dari OSM lines, filter ke
region Sulawesi (3 sub-sistem: Sulutgo, Sulteng, Sulselrabar).

Pola sama dengan extract_kalimantan_transmission.py: OSM `power=line` adalah
...
bbox-nya overlap di "pinggang" tengah pulau. assign_system memakai centroid
tiebreak: kalau centroid garis masuk >1 bbox, dipilih sistem dengan centroid
sistem terdekat.

Filter:
  - Hanya transmisi tegangan >= 70 kV (exclude distribusi 20 kV)
  - Garis yang centroid-nya jatuh di region Sulawesi

Output:
  - data/processed/transmission_master_sulawesi.csv
  - data/processed/transmission_sulawesi.geojson
"""
import re
import json
import csv
...
OUT_CSV = ROOT / "data/processed/transmission_master_sulawesi.csv"
OUT_GJ = ROOT / "data/processed/transmission_sulawesi.geojson"
...
# Sub-sistem listrik Sulawesi + bbox (lat_min, lon_min, lat_max, lon_max).
# Bbox = union provinsi penyusun sistem:
#   Sulutgo     = Sulawesi Utara + Gorontalo          (lengan utara)
#   Sulteng     = Sulawesi Tengah                     (tengah + lengan timur)
#   Sulselrabar = Sulsel + Sultra + Sulbar            (lengan barat daya + tenggara)
# lon_min 118.50: cukup menutup Sulbar tapi berhenti sebelum Kalimantan
# (Kaltim paling timur ~lon 117,6). lon_max 127.00: menutup Talaud tapi
# berhenti sebelum Maluku Utara (Ternate ~lon 127,4).
SYSTEMS = [
    ("Sulutgo",     ( 0.00, 121.10,  5.70, 127.00)),
    ("Sulteng",     (-3.10, 119.00,  1.50, 124.30)),
    ("Sulselrabar", (-7.50, 118.50, -0.85, 124.60)),
]

# Centroid tiebreaker per sistem (lat, lon) — dipakai kalau centroid garis
# jatuh di overlap bbox >1 sistem. Ditaruh di "pusat massa" jaringan tiap
# sistem: tengah lengan utara (Sulutgo), tengah Sulteng, dan koridor
# Makassar–Parepare (Sulselrabar).
SYSTEM_CENTROIDS = {
    "Sulutgo":     ( 0.90, 123.60),
    "Sulteng":     (-1.20, 121.00),
    "Sulselrabar": (-4.20, 120.20),
}


def assign_system(centroid_lat, centroid_lon):
    """Return nama sistem, atau None kalau di luar region Sulawesi.
...
def voltage_class(max_kv):
    ...
    Warna konsisten dengan docs/design_decisions.md. Backbone Sulawesi
    150 kV; 275 kV backbone sebagian masih dalam pembangunan; 500 kV
    belum ada di Sulawesi.
    """
    if max_kv >= 500:
...
def run():
    ...
    print(f"  Di luar region Sulawesi:  {skipped_outside}")
