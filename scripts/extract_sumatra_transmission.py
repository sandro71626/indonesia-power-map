"""
Build transmission_master_sumatra.csv + GeoJSON dari OSM lines, filter ke
region Sumatra (3 sistem: Sumatra mainland, Batam, Babel).

Pola sama dengan extract_jamali_transmission.py: OSM `power=line` adalah
...
Filter:
  ...
  - Garis yang centroid-nya jatuh di region Sumatra

Output:
  - data/processed/transmission_master_sumatra.csv
  - data/processed/transmission_sumatra.geojson

Lihat docs/naming_conventions.md untuk konvensi 'Sumatra' vs 'Sumatera'.
"""
import re
import json
import csv
...
OUT_CSV = ROOT / "data/processed/transmission_master_sumatra.csv"
OUT_GJ = ROOT / "data/processed/transmission_sumatra.geojson"
...
# Sistem listrik region Sumatra. Line di-assign berdasar centroid (rata-rata
# lat/lon semua titik). Bbox: (lat_min, lon_min, lat_max, lon_max).
#
# Urutan: pulau dengan sistem isolated (Batam, Babel) dicek DULU supaya line
# di pulau itu tidak keburu masuk bbox mainland yang lebih luas. Sumatra
# mainland adalah satu bbox besar; lon_max 105.40 berhenti sebelum Selat
# Bangka supaya tidak overlap dengan Babel.
SYSTEMS = [
    ("Batam",   (-0.20, 103.40,  1.55, 105.00)),
    ("Babel",   (-3.70, 105.50, -1.30, 108.60)),
    ("Sumatra", (-6.10,  94.80,  6.20, 105.40)),
]


def assign_system(centroid_lat, centroid_lon):
    """Return nama sistem, atau None kalau di luar region Sumatra."""
...
def voltage_class(max_kv):
    ...
    Warna konsisten dengan docs/design_decisions.md. Sumatra backbone 275 kV;
    500 kV belum eksisting tapi rule tetap disediakan untuk forward-compat.
    """
    if max_kv >= 500:
...
def run():
    ...
    print(f"  Di luar region Sumatra: {skipped_outside}")
    ...
    print("\nPer sistem:")
    print(f"  {'System':<10}{'Lines':>8}{'Length (km)':>15}")
    for s, _ in SYSTEMS:
        if count_by_system[s] == 0:
            continue
        print(f"  {s:<10}{count_by_system[s]:>8}{length_by_system[s]:>15,.0f}")
